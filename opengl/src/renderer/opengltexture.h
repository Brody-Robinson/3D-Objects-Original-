...
//all functions are specific for 2D textures
	class _2DTex : public opengltexture
	{
	public:
		_2DTex() = default;
		_2DTex(unsigned char* data);
		_2DTex(const std::string& texture_filePath, const std::string& name = "m_texture", unsigned int format = GL_RGBA, unsigned internalFormat = GL_RGBA8);
		_2DTex(glenum internalFormat, glenum format, int width, int hieght);
		~_2DTex();
...
//all functions are specific for  cubemaps
	...
	public:
		...
		openglCubeMaptexture(std::array<_2DTex, 6> textures);
