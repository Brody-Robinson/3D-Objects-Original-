...
{
	rotation =  glm::rotate(glm::mat4(1.0f), glm::radians(270.0f), glm::vec3(1.0f, 0.0f, 0.0f));
...
void Cylinder::rotatePoints()
{
	glm::vec3 Rpoints;
	for (int i = 0; i < vertices.size() / 3; i+=3)
	{
		Rpoints = rotation * glm::vec4(vertices[i], vertices[i+1], vertices[i+2], 1.0f );
		vertices[i] = Rpoints.x;
		vertices[i+1] = Rpoints.y;
		vertices[i+2] = Rpoints.z;
	}
}
